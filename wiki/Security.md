...
4. **Same-Host Redirect Validation:** During direct file streaming (`fileknot`, `pixeldrain`, `mediafire`, `workupload`), HTTP redirects are monitored. The target must remain on the exact same host or an authorized subdomain. Detours to foreign domains are rejected.
