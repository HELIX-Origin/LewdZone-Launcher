...
2. **Extract to a Permanent Directory:** Unpack the contents into a stable location, such as:
   ...
   C:\Utilities\7z\
   ```
   *(Ensure `7zz.exe` or `7za.exe` is inside this directory).*
3. **Configure the Path in Settings:**
   - Go to **Settings** (`/settings`) in the launcher.
   - Enter `C:\Utilities\7z` or `C:\Utilities\7z\7zz.exe` into the **7-Zip console executable path** field.
   - Alternatively via CLI: `lewdzone settings set 7z-path "C:\Utilities\7z\7zz.exe"`.
4. **Auto-Detection:** LewdZone Launcher automatically inspects `C:\Utilities\7z`, `C:\Program Files\7-Zip\7z.exe`, and your system `PATH`. Placing `7zz.exe` in `C:\Utilities\7z` allows it to be detected automatically!
...
### Explanation
- **Direct-file hosts** (`fileknot`, `pixeldrain`, `mediafire`, `workupload`) are streamed directly inside the app with byte counters and progress metrics.
- **Cloud-file hosts** (such as MEGA, Google Drive, Dropbox) intentionally hand the resolved URL to your **OS default handler** (installed desktop cloud app or default browser).
...
- **To reopen:** Click or double-click the LewdZone tray icon, or right-click the tray icon and choose **Show LewdZone**.
- **To exit completely:** Right-click the tray icon and select **Quit**.
